# Score preference pairs with a reward model and split into easy/hard curriculum sets
import os
import json
import torch
...
print("Loading reward model (DeBERTa-v3)...")
...
for row in tqdm(dataset):
    ...
    # large margin -> easy, small margin -> hard
    margin = score_chosen - score_rejected
    margins.append(margin)

dataset = dataset.add_column("margin", margins)

print("Partitioning into easy and hard curriculum...")
...
save_metadata(
    dataset, easy_dataset, hard_dataset,
    train_easy, train_hard, train_all,
    eval_easy, eval_hard, eval_all,
)
